...
async function getOrders() {
    ...
    while (loop < maximumLoop) {
        ...
        const orders = await Promise.all(data?.data?.order_data?.details_list.map(async (order) => getOrderDetail(order.info_card.order_id)))
...
async function getOrderDetail(orderId) {
    ...
    order.parcel = data.info_card?.parcel_cards?.map(parcel => {
        return {
            ...
            itemsGroup: parcel?.product_info?.item_groups?.map(({ items, num_items }) => {
                return {
                    num_items,
            ...
            payment_info: {
                ...
                merchandise_subtotal: getValueFromInfoRowsByLable(parcel?.payment_info.info_rows, "label_odp_merchandise_subtotal"),
                shipping: getValueFromInfoRowsByLable(parcel?.payment_info.info_rows, "label_odp_shipping"),
                shipping_discount_subtotal: getValueFromInfoRowsByLable(parcel?.payment_info.info_rows, 'label_odp_shipping_discount_subtotal'),
                shopee_voucher_applied: getValueFromInfoRowsByLable(parcel?.payment_info.info_rows, 'label_odp_shopee_voucher_applied'),
                redeemed_number_coins: getValueFromInfoRowsByLable(parcel?.payment_info.info_rows, 'label_odp_redeemed_number_coins'),
                shop_voucher_applied: getValueFromInfoRowsByLable(parcel?.payment_info.info_rows, 'label_odp_shop_voucher_applied'),
            },
        }
    })
    ...
    order.processing_info = {
        order_time: convertTime(getValueFromInfoRowsByLable(data?.processing_info.info_rows, 'label_odp_order_time')),
        payment_time: convertTime(getValueFromInfoRowsByLable(data?.processing_info.info_rows, 'label_odp_payment_time')),
        ship_time: convertTime(getValueFromInfoRowsByLable(data?.processing_info.info_rows, 'label_odp_ship_time')),
        completed_time: convertTime(getValueFromInfoRowsByLable(data?.processing_info.info_rows, 'label_odp_completed_time')),
    }
    return order
}
...
function getValueFromInfoRowsByLable(info_rows, lable) {
    return Number(info_rows.find(row => row.info_label.text == lable)?.info_value.value)
}
